...
HOLDINGS_COLUMNS = (
    ...
    "industry",
    "market",
    # 投資対象地域。market（上場市場）とは別軸＝東証上場のオルカン・S&P500 投信を
    # 「日本株」に計上しないための列。個別株は空欄でよい（market から導出する）
    "region",
    # 時価総額（円建て・自動取得）。企業規模の分散を見るために使う。
    # ETF・投信は yfinance が返さない（quoteType: ETF）ので空欄のまま＝対象外
    "market_cap",
    "div_per_share",
